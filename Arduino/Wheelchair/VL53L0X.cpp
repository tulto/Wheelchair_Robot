...
void VL53L0X::startRangeSingleMillimeters()
{
  writeReg(0x80, 0x01);
  writeReg(0xFF, 0x01);
  writeReg(0x00, 0x00);
...
bool VL53L0X::isSingleRangeMeasurementReady()
{ 
  iteration_since_measurement_start++;

  if(iteration_since_measurement_start >= max_iteration_for_measurement){
    iteration_since_measurement_start = 0;
    no_measurement_received_in_max_iter = true;
    return true;
  }
  if(measurementStarted && !(readReg(SYSRANGE_START) & 0x01)){
    iteration_since_measurement_start = 0;
    measurement_ready_to_read = true;
    return true;
  }
  else{
    return false;
...
uint16_t VL53L0X::readSingleRangeReadyMeasurementMillimeters()
{
  if(no_measurement_received_in_max_iter){
    iteration_since_measurement_start = 0;
    measurement_ready_to_read = false;
    no_measurement_received_in_max_iter = false;
    return 0;
  }

  if(measurement_ready_to_read){
    iteration_since_measurement_start = 0;
    measurement_ready_to_read = false;
    no_measurement_received_in_max_iter = false;
    return readRangeContinuousMillimeters();
